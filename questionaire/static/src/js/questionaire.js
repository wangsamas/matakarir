odoo.define('questionaire.questionaire', function (require) {
'use strict';
...
var _t = core._t;
/*
 * This file is intended to add interactivity to questionaire forms
 */

var the_form = $('.js_questionaireform');

if(!the_form.length) {
    return $.Deferred().reject("DOM doesn't contain '.js_questionaireform'");
}

    console.debug("[questionaire] Custom JS for questionaire is loading...");
    ...
    if (_.isUndefined(submit_controller)) {
        $(".js_questionaireform .input-group-addon span.fa-calendar").css("pointer-events", "none");
        $('.js_questionaireform :input').prop('disabled', true);
        print_mode = true;
    }
    ...
    $('.js_radio input[type="radio"][data-oe-questionaire-otherr!="1"]').click(function(){
        $(this).closest('.js_radio').find('.js_comments>input[type="text"]').val("");
    });
    $('.js_comments input[type="radio"]').click(function(){
        $(this).closest('.js_comments').find('>input[data-oe-questionaire-othert="1"]').focus();
    });
    // Custom code for right behavior of dropdown menu with comments
    $('.js_drop input[data-oe-questionaire-othert="1"]').hide();
    $('.js_drop select').change(function(){
        ...
        if($(this).val() === other_val){
            ...
            $(this).closest('.js_drop').find('input[data-oe-questionaire-othert="1"]').show().focus();
        }
        else{
            $(this).parent().removeClass('col-md-6').addClass('col-md-12');
            $(this).closest('.js_drop').find('input[data-oe-questionaire-othert="1"]').val("").hide();
        }
    });
    ...
    function prefill(){
        if (! _.isUndefined(prefill_controller)) {
            var prefill_def = $.ajax(prefill_controller, {dataType: "json"})
                ...
                .fail(function(){
                    console.warn("[questionaire] Unable to load prefill data");
                });
            return prefill_def;
        }
    }
    ...
    function display_scores(){
        if (! _.isUndefined(scores_controller)) {
            var score_def = $.ajax(scores_controller, {dataType: "json"})
                ...
                .fail(function(){
                    console.warn("[questionaire] Unable to load score data");
                });
            return score_def;
        }
    }
    ...
    //     function(){
    //         console.debug("[questionaire] Focus lost on question " + $(this).attr("id"));
    // });
    ...
    console.debug("[questionaire] Custom JS for questionaire loaded!");
