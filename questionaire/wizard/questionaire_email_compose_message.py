...
class QuestionaireMailComposeMessage(models.TransientModel):
    _name = 'questionaire.mail.compose.message'
    _inherit = 'mail.compose.message'
    _description = 'Email composition wizard for Questionaire'

    def default_questionaire_id(self):
        context = self.env.context
        if context.get('model') == 'questionaire.questionaire':
            return context.get('res_id')

    questionaire_id = fields.Many2one('questionaire.questionaire', string='Questionaire', default=default_questionaire_id, required=True)
    ...
    public_url = fields.Char(compute="_compute_questionaire_url", string="Public url")
    public_url_html = fields.Char(compute="_compute_questionaire_url", string="Public HTML web link")
    partner_ids = fields.Many2many('res.partner', 'questionaire_mail_compose_message_res_partner_rel', 'wizard_id', 'partner_id', string='Existing contacts')
    attachment_ids = fields.Many2many('ir.attachment', 'questionaire_mail_compose_message_ir_attachments_rel', 'wizard_id', 'attachment_id', string='Attachments')
    ...
    date_deadline = fields.Date(string="Deadline to which the invitation to respond is valid",
        help="Deadline to which the invitation to respond for this questionaire is valid. If the field is empty,\
        the invitation is still valid.")

    @api.depends('questionaire_id')
    def _compute_questionaire_url(self):
        for wizard in self:
            wizard.public_url = wizard.questionaire_id.public_url
            wizard.public_url_html = wizard.questionaire_id.public_url_html

    @api.model
    def default_get(self, fields):
        res = super(QuestionaireMailComposeMessage, self).default_get(fields)
        context = self.env.context
    ...
    def send_mail(self, auto_commit=False):
        ...
        QuestionaireUserInput = self.env['questionaire.user_input']
        ...
        def create_response_and_send_mail(wizard, token, partner_id, email):
            ...
            #set url
            url = wizard.questionaire_id.public_url
        ...
        def create_token(wizard, partner_id, email):
            if context.get("questionaire_resent_token"):
                questionaire_user_input = QuestionaireUserInput.search([('questionaire_id', '=', wizard.questionaire_id.id),
                    ('state', 'in', ['new', 'skip']), '|', ('partner_id', '=', partner_id),
                    ('email', '=', email)], limit=1)
                if questionaire_user_input:
                    return questionaire_user_input.token
            if wizard.public != 'email_private':
                return None
            else:
                token = pycompat.text_type(uuid.uuid4())
                # create response with token
                questionaire_user_input = QuestionaireUserInput.create({
                    'questionaire_id': wizard.questionaire_id.id,
        ...
        for wizard in self:
            ...
            if wizard.body.find("__URL__") < 0:
                raise UserError(_("The content of the text don't contain '__URL__'. \
                    __URL__ is automaticaly converted into the special url of the questionaire."))
